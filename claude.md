...
6. **Regenerate migrations if models changed** (see below)
7. **Commit**: `git commit -m "feat(investments): add investment tracking"`
8. **Pre-push hook validates schema** (automatic, see below)
9. **Push and create PR**

### **CRITICAL: Regenerating Migrations After Model Changes**

**ALWAYS do this after editing any model file** — schema drift causes 500 errors:
...
```bash
# Generate migration (runs after every model change!)
docker exec budget-flow-backend alembic revision --autogenerate -m "add investment accounts table"

# Review generated migration file
cat backend/alembic/versions/xxxx_add_investment_accounts.py

# Apply migration
docker exec budget-flow-backend alembic upgrade head

# Test rollback
docker exec budget-flow-backend alembic downgrade -1
docker exec budget-flow-backend alembic upgrade head

# Verify DB schema matches models
...
# Run full test suite
echo "  ▶ Running tests..."
docker exec budget-flow-backend python -m pytest tests/ -v --tb=short -q
if [ $? -ne 0 ]; then
...
docker exec budget-flow-backend alembic revision --autogenerate --dry-run -m "drift check" > /tmp/migration_check.txt
if grep -q "detected" /tmp/migration_check.txt; then
  echo "  ❌ Schema drift detected. Run:"
...
Before pushing to trigger CI, test locally using `act`:
...
act -j test          # Run unit + integration tests only
act -j lint          # Run code quality checks
act -j e2e           # Run end-to-end tests (requires docker-compose)
act                  # Run all jobs
```

**Why use this:** Catch CI failures before pushing (saves time).
